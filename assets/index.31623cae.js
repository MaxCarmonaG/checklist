 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function(e){if("L"===e.limitType&&0===e.explicitOrderBy.length)throw new tX(tW.UNIMPLEMENTED,"limitToLast() queries require specifying at least one orderBy() clause")}(e._query),(function(e,t,r={}){let n=new tY;return e.asyncQueue.enqueueAndForget(async()=>(function(e,t,r,n,i){let s=new ok({next:r=>{s.eu(),t.enqueueAndForget(()=>s3(e,o)),r.fromCache&&"server"===n.source?i.reject(new tX(tW.UNAVAILABLE,'Failed to get documents from server. (However, these documents may exist in the local cache. Run again without setting source to "server" to retrieve the cached documents.)')):i.resolve(r)},error:e=>i.reject(e)}),o=new s7(r,s,{includeMetadataChanges:!0,ua:!0});return s6(e,o)})(await oP(e),e.asyncQueue,t,r,n)),n.promise})(r,e._query).then(r=>new ac(t,n,e,r)))}(oW(ad,"todos"));let t=[];return e.forEach(e=>{t.push(e)}),t}const am=document.getElementById("taskInput"),ay=document.getElementById("addTaskBtn"),av=document.getElementById("taskList");async function aw(){let e=await ag();av.innerHTML="",e.forEach((e,t)=>{if(!e.data().completed){let t=document.createElement("li");t.id=e.id,t.textContent=e.data().text,av.appendChild(t)}})}function ab(e){let t=document.createElement("div");return t.textContent=e,t.innerHTML}ay.addEventListener("click",()=>{let e=am.value.trim();if(e){let t=document.createElement("li");t.textContent=e,av.appendChild(t),am.value=""}}),av.addEventListener("click",e=>{"LI"===e.target.tagName&&e.target.remove()}),ay.addEventListener("click",async()=>{if(am.value.trim()){let e=document.getElementById("taskInput"),t=ab(e.value.trim());t&&(await ap(t),aw(),e.value=""),aw()}}),ab(am.value.trim()),window.addEventListener("error",e=>console.error("Error occurred: ",e.message));var aE={};aE=new URL("service-worker.e42dd0ab.js",import.meta.url).toString();const a_=new URL(aE);"serviceWorker"in navigator&&navigator.serviceWorker.register(a_.href,{scope:"/checklist/"}).then(e=>console.log("Service Worker Registered for scope:",a_.href,"with","file:///app.js")).catch(e=>console.error("Service Worker Error:",e));
//# sourceMappingURL=index.31623cae.js.map
